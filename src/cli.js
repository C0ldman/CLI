	...
	.action((id) => {
		...
		if (commander.html) {
			...
			tags.forEach((element) => {
				...
				if (tag == 'coimage' && get.imagesFileNames(id).includes(element.attribs.id)) {
					image.getImage(id, get.imageNameWithDimension(id, element.attribs.id))
						.then((img) => {
							let width = img.width / 2, height = img.height / 2;
							if ((image.isOdd(img.width) || image.isOdd(img.height)) && !commander.size) {
